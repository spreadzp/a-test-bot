...
module.exports = function (s, conf) {
    ...
    function startClient(close, currency, volume, socketName) {
      closePrice = close;
      currentCurrency = currency;
      currentVolume = volume; 
      nameSocket = socketName;
      console.log(' nameSocket:',nameSocket );
      ipc.connectTo( 
        nameSocket,
        socketPath,
        connecting
  ...
  function connecting(socket) { 
   ipc.of[nameSocket].emit(
    'currentDate',
    {
         message: `${nameSocket}, ${currentCurrency}, ${closePrice}, ${currentVolume}`
    }
  ); 
  ...
  function writeReport (is_progress, blink_off) {
    ...
    startClient(s.period.close, s.currency, s.period.volume, nameForSocket);
