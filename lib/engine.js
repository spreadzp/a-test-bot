...
const { URL_SERVER, URL_BOT_SERVER } = require('./../url-config.json');
const currExchange = 'Bittrex';
...
module.exports = function (s, conf) {
  ...
  const server = new net.Server(function (socket) {
    server.on('error', () => {
      setTimeout(() => startServerConnection(), 5000);
    })
  ...
  /*   const totalUrl = `tcp://${URL_BOT_SERVER}:${serverPort}`;
    console.log('totalUrl :', totalUrl); */
  //server.listen(serverPort, URL_BOT_SERVER)
  ...
  function startConnection() {
    console.log('client try connecting to port :', URL_SERVER);
    client.reconnect()
    //client.options.retryDelay = 2000;
  }
  ...
  function writeReport(is_progress, blink_off) {
    ...
    //console.log('s.exchange :', s.trades[s.trades.length-1].selector );
    client.request('Port', { "serverPort": serverPort, "url": URL_BOT_SERVER, "exchange": currExchange, "time": Date.now() })((err, res) => {
      //console.log(err, res) // null { a: 4 }
      let diff;
      if (res) {
        diff = Date.now() - res.time;
      }

      nameForSocket = nameForSocket.replace(".", " ");
      client.notification(nameForSocket, [s.period.close, s.currency, s.period.volume, diff, Date.now()])
      //client.destroy()
      //server.close()
    })


    client.notification(nameForSocket, [s.period.close, s.currency, s.period.volume])
    startConnection(s.period.close, s.currency, s.period.volume, nameForSocket);
